...
RUN apt-get update && apt-get install -y libopencv-dev openimageio-tools libopenimageio-dev && \
    git clone https://github.com/adobe/USD-Fileformat-plugins && \
    cd USD-Fileformat-plugins && \
	cmake -S . -B ./build -DCMAKE_INSTALL_PREFIX=bin -DUSD_FILEFORMATS_ENABLE_CXX11_ABI=ON -Dpxr_ROOT=${USD_BUILD_PATH} -DUSD_FILEFORMATS_ENABLE_FBX=false -DUSD_FILEFORMATS_ENABLE_GLTF=false -DUSD_FILEFORMATS_ENABLE_PLY=false && \ 
